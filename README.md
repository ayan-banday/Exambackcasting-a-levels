...
Subjects: **CSE, Maths, GP, Chemistry, Biology**, each at **H2 (current focus)** and **H1 (later)**.
...
| `system/prompts/` | The live entry-point prompts Claude follows (behavior, subject-guide, topic-session) — also where you drop the CSE example guide | Set up (don't edit) |
...
| Your **CSE gold-standard example guide** | `system/prompts/` (as the quality bar) **and/or** `CSE/H2/subject_examiner_guide.md` (to use as the real guide) — see `system/prompts/_README.md` |
...
- *"Build the CSE H2 subject guide from the inputs."* → Claude runs playbook 01.
...
`STATUS: NOT BUILT`. Next steps: (1) your assistant loads `inputs/`, (2) you drop the CSE example
